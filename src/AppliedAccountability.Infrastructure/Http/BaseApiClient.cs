...
namespace AppliedAccountability.Infrastructure.Http;
...
public abstract class BaseApiClient
{
    ...
    /// <summary>
    /// Helper method to safely extract string values from dynamic objects
    /// </summary>
    protected string? GetStringValue(Dictionary<string, object> data, string key)
    {
        return data.TryGetValue(key, out var value) ? value?.ToString() : null;
    }

    /// <summary>
    /// Helper method to safely extract DateTime values from dynamic objects
    /// Returns DateTime with UTC kind to ensure PostgreSQL compatibility
    /// </summary>
    protected DateTime? GetDateTimeValue(Dictionary<string, object> data, string key)
    {
        if (data.TryGetValue(key, out var value) && value != null)
        {
            if (DateTime.TryParse(value.ToString(), out var dateTime))
            {
                // PostgreSQL requires DateTime values with UTC kind for timestamptz columns
                // Convert to UTC if not already specified
    ...
    /// <summary>
    /// Helper method to safely extract int values from dynamic objects
    /// </summary>
    protected int? GetIntValue(Dictionary<string, object> data, string key)
    {
        if (data.TryGetValue(key, out var value) && value != null)
        {
    ...
    /// <summary>
    /// Helper method to safely extract decimal values from dynamic objects
    /// </summary>
    protected decimal? GetDecimalValue(Dictionary<string, object> data, string key)
    {
        if (data.TryGetValue(key, out var value) && value != null)
        {
    ...
    /// <summary>
    /// Helper method to safely extract bool values from dynamic objects
    /// </summary>
    protected bool? GetBoolValue(Dictionary<string, object> data, string key)
    {
        if (data.TryGetValue(key, out var value) && value != null)
        {
